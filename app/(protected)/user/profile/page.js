...
//   const auth = getAuth();
//   async const user = auth.currentUser;
...
export default function ProfileForm() {
  ...
  useEffect(() => {
    // Nasłuchuj zmian w stanie użytkownika
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (!currentUser) {
        router.push("/user/login"); // Przekieruj na stronę logowania, jeśli brak użytkownika
      } else {
        setUser(currentUser);
        try {
          // Pobierz dane adresowe użytkownika
    ...
    return () => unsubscribe(); // Sprzątaj nasłuchiwanie
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setError("");
      router.push("/"); // Przekieruj po zapisaniu
    } catch (e) {
      console.error("Error updating profile:", e);
      setError("Failed to update profile.");
    }
